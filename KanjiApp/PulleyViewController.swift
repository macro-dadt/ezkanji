...
open class PulleyViewController: UIViewController {
    let appDelegate = UIApplication.shared.delegate as! AppDelegate
     ...
     static var pulleyViewControllerDelegate: PulleyViewController!
    // Interface Builder
    ...
    @IBInspectable public var topInset: CGFloat = 65.0 {
        didSet {
            if self.isViewLoaded
            {
                self.view.setNeedsLayout()
    ...
    static var willShowCategoryToolBar = true
    var bottomVC:BottomViewController!
    var bottom2VC:Bottom2ViewController!
    
    override open func viewDidLoad() {
        super.viewDidLoad()
        print("puley","viewdidload")
        PulleyViewController.pulleyViewControllerDelegate = self
       
        bottomVC = storyboard?.instantiateViewController(withIdentifier: "bottom") as? BottomViewController
        bottomVC?.view.frame = CGRect(x: 0, y: self.view.frame.height - 44, width: self.view.frame.width, height: 44)
        bottom2VC = storyboard?.instantiateViewController(withIdentifier: "bottom2") as? Bottom2ViewController
        bottom2VC?.view.frame = CGRect(x: 0, y: self.view.frame.height - 44, width: self.view.frame.width, height: 44)
        ...
        {
            assert(mainView != nil && bottomView != nil , "When instantiating from Interface Builder you must provide container views with an embedded view controller.")
            
            // Locate main content VC
            for child in self.childViewControllers
            {
                if child.view == mainView.subviews.first
                {
                    primaryContentViewController = child
                }
                
                if child.view == bottomView.subviews.first
                {
...
extension PulleyViewController: UIScrollViewDelegate {
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if scrollView == drawerScrollView
        {
            ...
            (primaryContentViewController as? PulleyPrimaryContentControllerDelegate)?.drawerChangedDistanceFromBottom?(drawer: self, distance: scrollView.contentOffset.y + lowestStop + 50)
        }
    }
}
